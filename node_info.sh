...
while test $# -gt 0; do
	...
	-h|--help)
		...
		echo
		echo -e "${C_LGn}Functionality${RES}: the script shows information about an IDEP node"
		echo
		...
		echo -e "https://github.com/SecorD0/IDEP/blob/main/node_info.sh - script URL"
...
if [ -n "$idep_wallet_name" ] && [ ! -n "$idep_wallet_address" ]; then
	printf_n "$t_ewa"
	idep_wallet_address=`$daemon keys show "$idep_wallet_name" -a`
	if [ -n "$idep_wallet_address" ]; then
		. <(wget -qO- https://raw.githubusercontent.com/SecorD0/utils/main/miscellaneous/insert_variable.sh) -n idep_wallet_address -v "$idep_wallet_address"
	else
		printf_n "$t_ewa_err"
	fi
fi
...
delegated=`jq -r ".tokens" <<< $node_info`
...
"$voting_power"
	...
	if [ "$catching_up" = "true" ]; then
		#current_block=`wget -qO- 123/block | jq -r ".result.block.header.height"`
	...
	if [ -n "$idep_wallet_address" ]; then
		...
		balance=`$daemon query bank balances "$idep_wallet_address" -o json --node "$node_tcp" | jq -r ".balances[0].amount"`
